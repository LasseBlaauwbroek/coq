...
let compute_bl_goal env handle ind lnamesparrec nparrec =
  let eqI = eqI handle ind lnamesparrec in
  ...
  let create_input c =
    ...
    in
      ...
      let u = Univ.Instance.empty in
     create_input (
        mkNamedProd (make_annot n Sorts.Relevant) (mkFullInd env (ind,u) nparrec) (
          mkNamedProd (make_annot m Sorts.Relevant) (mkFullInd env (ind,u) (nparrec+1)) (
            mkArrow
              (mkApp(eq (),[|bb ();mkApp(eqI,[|mkVar n;mkVar m|]);tt ()|]))
              Sorts.Relevant
              (mkApp(eq (),[|mkFullInd env (ind,u) (nparrec+3);mkVar n;mkVar m|]))
        )))
...
let make_bl_scheme env handle mind =
  ...
  let ctx = UState.make ~lbound:(Environ.universes_lbound env) (Environ.universes env) in
  let bl_goal = EConstr.of_constr bl_goal in
  let (ans, _, ctx) = Pfedit.build_by_tactic ~poly:false ~side_eff:false env ctx bl_goal
    ...
    in
      ...
      create_input (
        mkNamedProd (make_annot n Sorts.Relevant) (mkFullInd env (ind,u) nparrec) (
          mkNamedProd (make_annot m Sorts.Relevant) (mkFullInd env (ind,u) (nparrec+1)) (
            mkArrow
              (mkApp(eq,[|mkFullInd env (ind,u) (nparrec+2);mkVar n;mkVar m|]))
              Sorts.Relevant
              (mkApp(eq,[|bb;mkApp(eqI,[|mkVar n;mkVar m|]);tt|]))
        )))
...
let make_lb_scheme env handle mind =
  ...
  let (ans, _, ctx) = Pfedit.build_by_tactic ~poly:false ~side_eff:false env ctx lb_goal
...
let compute_dec_goal env ind lnamesparrec nparrec =
  ...
  let avoid = List.fold_right (Nameops.Name.fold_right (fun id l -> Id.Set.add id l)) (List.map RelDecl.get_name lnamesparrec) Id.Set.empty in
    ...
    in
      let n = next_ident_away (Id.of_string "x") avoid and
          m = next_ident_away (Id.of_string "y") avoid in
        let eqnm = mkApp(eq,[|mkFullInd env ind (2*nparrec+2);mkVar n;mkVar m|]) in
        create_input (
          mkNamedProd (make_annot n Sorts.Relevant) (mkFullInd env ind (2*nparrec)) (
            mkNamedProd (make_annot m Sorts.Relevant) (mkFullInd env ind (2*nparrec+1)) (
...
let make_eq_decidability env handle mind =
  ...
  let lnonparrec,lnamesparrec =
    context_chop (nparams-nparrec) mib.mind_params_ctxt in
  let (ans, _, ctx) = Pfedit.build_by_tactic ~poly:false ~side_eff:false env ctx
    (EConstr.of_constr (compute_dec_goal env (ind,u) lnamesparrec nparrec))
